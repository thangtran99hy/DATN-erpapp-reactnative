...
const InvoiceForm = (props) => {
    ...
    const onSave = async () => {
        ...
        formData.append('note', dataForm.note);
        formData.append('exportDate', dataForm.exportDate ? moment(dataForm.exportDate).format('YYYY-MM-DD') :'');
        formData.append('expiryDate', dataForm.expiryDate ? moment(dataForm.expiryDate).format('YYYY-MM-DD') :'');
        ...
        if (action === ACTION_EDIT) {
            ...
            const res = await invoiceApi.editInvoiceById(editId, formData);
            if (res.status === 200) {
    ...
    return (
        ...
        >
            ...
            <FormItem
                ...
                input={
                    <DatePickerCustom
                        date={dataForm?.exportDate ? new Date(dataForm?.exportDate) : null}
                        onChange={(date) => {
                            setDataForm(prev => ({
                                ...prev,
                                exportDate: date
            ...
            <FormItem
                ...
                input={
                    <DatePickerCustom
                        date={dataForm?.expiryDate ? new Date(dataForm?.expiryDate) : null}
                        onChange={(date) => {
                            setDataForm(prev => ({
                                ...prev,
                                expiryDate: date
                                ...
                                <View style={styles.itemCellView}>
                                    ...
                                    <View style={styles.itemCell2}>
                                        <Input
                                            ...
                                            editable={false}
                                            placeholder={t('invoice.field.price')}
                                        />
                                    </View>
                                    <View style={styles.itemCell2}>
                                        <Input
